...
let rec extend_env env xs vs =
  ...
  | [], [] -> env
  | x::xs, (env',e)::vs ->
     extend_env ((x, ref (VClosure (env', e))) :: env) xs vs
  | _, _ -> runtime_error "Bad pattern"
...
let rec interp env = function
  ...
  | Syntax.Fst e ->
      (match interp env e with
       | VClosure (env', Syntax.Pair (e1, _)) -> interp env' e1
       | _ -> runtime_error "Pair expected in fst")
  | Syntax.Snd e ->
      (match interp env e with
       | VClosure (env', Syntax.Pair (_, e2)) -> interp env' e2
